...
# Setup Phase: Upload Frame if it doesn't exist
FRAME_PATH = "frame.png"

if not os.path.exists(FRAME_PATH):
    st.warning("Initial Setup: System Frame is missing.")
    st.info("Please upload the transparent ABLBL frame PNG once to configure the agent.")
    setup_frame = st.file_uploader("Upload System Frame", type=["png"], key="setup")
    
    if setup_frame is not None:
        with open(FRAME_PATH, "wb") as f:
            f.write(setup_frame.getbuffer())
        st.success("Frame configured successfully! Please refresh the page.")
        st.rerun()
else:
    # Main App Phase
